...
void* mlx_int_new_xshm_image(t_xvar* xvar, int width, int height, int format)
{
	...
	img->data  = 0;
	img->image = XShmCreateImage(
	xvar->display, xvar->visual, xvar->depth, format, img->data, &(img->shm), width, height);
	if (!img->image)
	{
		free(img);
		return ((void*)0);
	...
	{
		img->pix  = XShmCreatePixmap(xvar->display,
                                    xvar->root,
                                    img->shm.shmaddr,
                                    &(img->shm),
                                    width,
...
void* mlx_int_new_image(t_xvar* xvar, int width, int height, int format)
{
	...
	if (!(img = malloc(sizeof(*img))) || !(img->data = malloc((width + 32) * height * 4)))
		return ((void*)0);
	bzero(img->data, (width + 32) * height * 4);
	img->image = XCreateImage(
	xvar->display, xvar->visual, xvar->depth, format, 0, img->data, width, height, 32, 0);
	if (!img->image)
	{
		free(img->data);
		free(img);
